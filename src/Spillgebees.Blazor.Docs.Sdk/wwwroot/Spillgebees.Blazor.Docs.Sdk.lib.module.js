...
var cshtmlRazor;
var hasRequiredCshtmlRazor;
function requireCshtmlRazor() {
  if (hasRequiredCshtmlRazor) return cshtmlRazor;
  hasRequiredCshtmlRazor = 1;
  cshtmlRazor = function(hljs) {
    ...
    const razorCodeBlockVariants = ["for", "if", "switch", "while", "using", "lock", "foreach"].map((keyword) => ({
      begin: `@${keyword}(?![\\w\\d])[^{]*\\{`,
      end: "}"
    }));
    const elseVariants = [
...
var cshtmlRazorExports = requireCshtmlRazor();
const razor = /* @__PURE__ */ getDefaultExportFromCjs(cshtmlRazorExports);
HighlightJS.registerLanguage("csharp", csharp);
HighlightJS.registerLanguage("cs", csharp);
...
HighlightJS.registerLanguage("cshtml-razor", razor);
HighlightJS.registerLanguage("razor", razor);
...
function afterWebStarted() {
  initialize();
}
function beforeWebAssemblyStart() {
  initialize();
}
function afterWebAssemblyStarted() {
  initialize();
}
function beforeServerStart() {
  initialize();
}
...
//# sourceMappingURL=Spillgebees.Blazor.Docs.Sdk.lib.module.js.map
